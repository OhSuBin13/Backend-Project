...
import (
	...
	"github.com/OhSuBin13/Backend-project/tree/main/cache-proxy/internal/cache"
)
...
//	X-Cache : HIT
//
// If No -> Forward the request to the origin server
//
//		Cache the response
//		Set the Headers
//		X-Cache : MISS
...
func (p *ProxyObject) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if c, ok := p.Cache[CACHE_KEY]; ok {
		p.Mutex.RUnlock()
		RespondWithHeaders(w, *c.Response, c.ResponseBody, "HIT", "CACHED_KEY")
		return
	}
	p.Mutex.RUnlock()
