...
public class OrderService {
    ...
    public Map<String, String> save(OrderRequest orderRequest, Authentication authentication) throws IOException, StripeException {
        User currentUser = (User) authentication.getPrincipal();

        // User is logged in, so already exists => just call .get() on optional to retrieve Hibernate-managed entity
        currentUser = userRepository.findWithAssociationById(currentUser.getId()).get();

        Address billingAddress = addressRepository.findWithAssociationById(orderRequest.getBillingAddressId())
                .orElseThrow(() -> new ResourceNotFoundException("No billing address found with ID: " + orderRequest.getBillingAddressId()));
        Address deliveryAddress = addressRepository.findWithAssociationById(orderRequest.getDeliveryAddressId())
        ...
        if (!billingAddress.getUser().getId().equals(currentUser.getId()) ||
            ...
            throw new APIException("One or both of the specified address are related to another user");
    ...
    public void fulfillOrder(String sessionId) throws StripeException, MessagingException {
        ...
        asyncFulfilmentService.finalizeOrder(order, orderProducts);
    }
    ...
    public void markOrderItemAsDelivered(Integer orderId, Integer productId, Authentication authentication) {
        ...
        if (orderItem.getStatus() != OrderItemStatus.SHIPPED)
            throw new APIException("Product with ID: " + orderItem.getProduct().getId() + " has not been shipped");
    ...
    public OrderStatsResponse calculateStatistics() {
        ...
        return orderRepository.calculateStatistic(from, to);
    }
}
