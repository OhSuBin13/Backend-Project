...
import static com.osb.shopapp.common.AppConstants.ACCEPTED_IMAGE_EXTENSIONS;
import static com.osb.shopapp.common.AppConstants.FLYWAY_USERS_NUMBER;
...
public class UserService {
    ...
    public UserResponse findById(Integer userId) {
        return userRepository.findWithAssociationById(userId)
    ...
    public UserResponse update(UserRequest userRequest, Integer userId, Authentication authentication) {
        ...
        currentUser = userRepository.findWithAssociationById(userId).get();
    ...
    public UserResponse uploadUserImage(MultipartFile image, Integer userId, Authentication authentication) throws IOException {
        ...
        // User is logged in, so already exists => just call .get() on optinal to retrieve Hibernate-managed entity
        currentUser = userRepository.findWithAssociationById(userId).get();
    ...
    public WishListItemResponse saveWishListItem(
        ...
        // User is logged in, so already exist => just call .get() on optional to retrieve Hibernate-managed entity
        currentUser = userRepository.findById(userId).get();
